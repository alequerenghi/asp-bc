...
def parse_file(filepath) -> tuple[int, int, NDArray[np.int64], float, float, NDArray[np.float64]]:
    ...
    D_lines = [line.strip() for line in D_block.strip().splitlines() if line.strip()]
    D = [[int(x) for x in re.split(r"\s+", line)] for line in D_lines]

    W_block = re.search(r"w:\[(.*?)\]", instance, re.S).group(1)
    W_lines = [line.strip() for line in W_block.strip().splitlines() if line.strip()]
    W = [[float(x) for x in re.split(r"\s+", line)] for line in W_lines]

    job_durations = np.array(D)[:, 0]
    energy_requirements = np.array(W)[:, 0]

    assert len(job_durations) == job_number, f"Mismatch: expected {job_number} jobs, got {len(job_durations)}"
    assert len(energy_requirements) == job_number, f"Mismatch: expected {job_number} jobs, got {len(energy_requirements)}"
